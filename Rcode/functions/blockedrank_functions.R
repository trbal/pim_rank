blockedrank.pim <- function(formula, data, h0 = 0.5,...){
...
summary.blockedrank.pim <- function(object,method,
                                 ...){
...
print.blockedrank.pim.summary <- function(x, digits = max(3L, getOption("digits") - 3L),...){
  orig <- paste(deparse(formula(x@formula)))
  cat("Summary of following PIM : \n K-sample rank test with blocks \n\nFormula: ", orig, "\n\n")
...
setMethod("show",
          "blockedrank.pim.summary",
          function(object){
            print.blockedrank.pim.summary(object)
          })


print.blockedrank.pim <- function(x, digits = max(3L, getOption("digits") - 3L),
                               ...){
...
setMethod('print',
          'blockedrank.pim',
          print.blockedrank.pim)

coef.blockedrank.pim <- coef.blockedrank.pim.summary <- function(object,...){
  object@coef
}

setMethod('coef',
          'blockedrank.pim',
          coef.blockedrank.pim)


setMethod('coef',
          'blockedrank.pim.summary',
          coef.blockedrank.pim.summary)


confint.blockedrank.pim <- function(object, parm, level = 0.95, method, ...){
...
confint.blockedrank.pim.summary <- function(object, parm, level = 0.95, method,...){
  # This code is almost literally copied from confint.default
  # because that one doesn't work with S4
  # kinda stupid, but well... 
  cf <- coef(object)
