friedman.pim <- function(formula, data, h0 = 0.5,...){
  ...
  if (any(table(eval(parse(text = paste("data$",groups, sep=""))), 
                eval(parse(text = paste("data$",blocks, sep="")))) != 1))
...
summary.friedman.pim <- function(object,method,
                                   ...){
...
print.friedman.pim.summary <- function(x, digits = max(3L, getOption("digits") - 3L),...){
...
setMethod("show",
          "friedman.pim.summary",
          function(object){
            print.friedman.pim.summary(object)
          })


print.friedman.pim <- function(x, digits = max(3L, getOption("digits") - 3L),
                                 ...){
...
coef.friedman.pim <- coef.friedman.pim.summary <- function(object,...){
  object@coef
}
...
setMethod('coef',
          'friedman.pim.summary',
          coef.friedman.pim.summary)


confint.friedman.pim <- function(object, parm, level = 0.95, method, ...){
...
confint.friedman.pim.summary <- function(object, parm, level = 0.95, method,...){
  # This code is almost literally copied from confint.default
  # because that one doesn't work with S4
  # kinda stupid, but well... 
  cf <- coef(object)
...
setMethod('confint',
          'friedman.pim',
          confint.friedman.pim)

setMethod('confint',
          'friedman.pim.summary',
          confint.friedman.pim.summary)
